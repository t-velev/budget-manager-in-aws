...
def lambda_handler(event, context):
    ...
    new_data_filter = ['Име', 'Тип', 'Сметка', 'Категория', 'Необходимост', 'Падеж', 'Архивирай']

    # A list of notion db column names to be filtered. Empty list filters nothing.
    id_cols_filter = ['Име']

    # Call the main extract function in ntn_utils.py
    status = run_full_extraction_pipeline(event, pg_table_name, subcategory_db_id, dag_name, task_name, map_all_data, map_filtered_data, new_data_filter, id_cols_filter)
